...
const App = () => {
    ...
    while (day <= endDate) {
      for (let i = 0; i < 7; i++) {
        ...
        days.push(
          <div
            key={day}
            className={`min-h-[70px] md:min-h-[120px] p-1 md:p-2 border-r border-b border-gray-800 transition-colors relative ${
              !isSameMonth(day, monthStart) ? 'bg-gray-900/40 text-gray-600' : 'bg-gray-800/20 text-gray-300'
            }`}
          >
            <span className={`text-[10px] md:text-sm font-medium ${isSameDay(day, new Date()) ? 'bg-blue-600 px-1.5 py-0.5 rounded text-white' : ''}`}>
              {formattedDate}
            </span>
              ...
              {dayTournaments.map((t, idx) => (
                <div 
                  ...
                  className={`p-1 md:p-1.5 rounded cursor-pointer hover:brightness-125 transition-all border-l-2 md:border-l-4 ${
                    ...
                    t.category === 'DPF1000' ? 'bg-red-600 border-white text-white' :
                    t.category === 'Special' ? 'bg-orange-600/60 border-orange-400 text-white shadow-sm' :
                    'bg-gray-700/40 border-gray-400 text-gray-200'
                  }`}
                >
                  <div className="hidden md:block text-[10px] font-bold truncate leading-tight">{t.name}</div>
                  <div className="block md:hidden w-full h-1.5 md:h-auto rounded-full bg-current opacity-60"></div>
                </div>
              ))}
            </div>
          </div>
  ...
  const Modal = () => {
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center p-0 md:p-4 bg-black/80 backdrop-blur-sm" onClick={closeModal}>
        <div className="bg-gray-800 w-full max-w-lg rounded-t-2xl md:rounded-2xl border-t md:border border-gray-700 shadow-2xl overflow-hidden max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
          <div className={`h-2 ${
          ...
          <div className="p-6 md:p-8">
            <div className="flex justify-between items-start mb-6">
              <div className="pr-8">
                <span className="text-[10px] md:text-xs font-bold uppercase tracking-widest text-gray-400 mb-1 block">{selectedEvent.category}</span>
                <h2 className="text-xl md:text-3xl font-black text-white leading-tight">{selectedEvent.name}</h2>
              </div>
              <button onClick={closeModal} className="p-2 hover:bg-gray-700 rounded-full text-gray-400 transition-colors flex-shrink-0">
                <X size={24} />
              </button>
            </div>
            ...
            <div className="space-y-4 md:space-y-6">
              <div className="flex items-start gap-3 text-gray-300">
                ...
                <div className="pt-0.5">
                  <p className="text-[10px] md:text-xs text-gray-500 font-bold uppercase">Lokation</p>
                  <p className="text-sm md:text-base font-medium">{selectedEvent.location}</p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3 text-gray-300">
                ...
                <div className="pt-0.5">
                  <p className="text-[10px] md:text-xs text-gray-500 font-bold uppercase">Beskrivelse</p>
                  <p className="text-xs md:text-sm text-gray-400 leading-relaxed">{selectedEvent.description || "Ingen beskrivelse tilgængelig."}</p>
                </div>
              </div>
